...
export class AuthService {
    ...
    async login(dto: LoginUsuarioDto): Promise<any> {
        ...
        const usuario = await this.authRepository.findOne({ where: [{ correo: correo }, { identificacion: identificacion }] });
        ...
        if (!contraseniaOk) return new UnauthorizedException(new MessageDto('Contrasenia erronea'));
    ...
    async updateUsuario(id_usuario: number, usuario: UpdateUsuarioDto){
        this.authRepository.update({id_usuario}, usuario)
    }
